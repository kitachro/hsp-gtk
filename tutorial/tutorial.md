...
　そして、最後の4つ目の引数ですが、ここには、コールバック関数が呼び出される時に引数として受け取りたいデータが入った変数のポインタを指定できます。変数のポインタを取得するには、varptr関数を使い、*varptr( data )*のように記述します。

特に受け渡したいデータがない場合には、無効なポインタを表す0を指定してください。
